...
class QueryResolver {
    ...
    static resolve(query) {
        ...
        if (appleMusicTrack.test(query))
            return types_1.QueryType.APPLE_MUSIC_TRACK;
        if (appleMusicPlaylist.test(query))
            return types_1.QueryType.APPLE_MUSIC_PLAYLIST;
        if (appleMusicAlbum.test(query))
            return types_1.QueryType.APPLE_MUSIC_ALBUM;
